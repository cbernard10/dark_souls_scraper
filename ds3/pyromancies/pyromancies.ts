...
const BASE_URL = "https://darksouls3.wiki.fextralife.com/Pyromancies";
...
const getMiracleData = async (cheerioRoot: cheerio.Root): Promise<Miracle> => {
  ...
  for (const row of rows) {
    ...
    const FP = $(row).find("td").eq(2).text().trim();
    const slots = $(row).find("td").eq(3).text().trim();
    const int = $(row).find("td").eq(4).text().trim();
    const fth = $(row).find("td").eq(5).text().trim();
